...
export class BuildingBlockRenderer {
	...
	#resolveActionFromHandler(handler) {
		...
		if (typeof handler === "object") {
			...
			return {
				actionName,
				payload: handler.payload ?? handler.actionPayload ?? undefined,
	...
	#resolveActionFromProps(props = {}) {
		...
		return {
			...
			payload:
				...
				props.props?.payload ??
				undefined,
			entityId: props.entityId ?? props.props?.entityId ?? undefined,
		};
	}
	...
	#delegateAction(element, descriptor) {
		...
		if (descriptor.payload !== undefined) {
			try {
				...
				try {
					element.dataset.actionPayload = String(descriptor.payload);
				} catch {
					void 0;
				}
			}
	...
	#extractBindingDescriptor(props = {}) {
		...
		const optionCandidate =
			(typeof props.bindingOptions === "object" &&
				props.bindingOptions) ||
	...
	#renderComposition(composition, context = {}) {
		...
		const entityRef = props.entityId ?? props.props?.entityId ?? undefined;
	...
	#applyPositioning(element, position) {
		...
		if (position.grid) {
			element.style.gridColumn = `${position.grid.column} / span ${
				position.grid.width || 1
			}`;
			element.style.gridRow = `${position.grid.row} / span ${
				position.grid.height || 1
			}`;
		}
	...
	#createErrorElement(message) {
		// Previously would create a forensic envelope; community build logs to console

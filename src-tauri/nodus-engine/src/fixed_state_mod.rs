...
use crate::license::{LicenseManager, LicenseTier, PluginAccessMode};
...
impl AppState {
    ...
    pub async fn new_community(license_manager: LicenseManager) -> Result<Self, AppStateError> {
    ...
    pub async fn initialize(&mut self) -> Result<(), AppStateError> {
        ...
        // Validate license
        if !self.license_manager.is_valid().await {
            return Err(AppStateError::InvalidLicense);
        }
        ...
        match license_tier {
            LicenseTier::Community => {
                ...
                // Community gets full app but with unsigned plugins allowed
            }
            LicenseTier::Pro => {
                tracing::info!("💼 Pro features initialized");  
                // Pro gets community + AI features
            }
            LicenseTier::Team => {
                tracing::info!("👥 Team features initialized");
                // Team gets pro + collaboration features  
            }
            LicenseTier::Enterprise => {
                tracing::info!("🏢 Enterprise features initialized");
                // Enterprise gets team + enterprise features + signed plugins only
            }
        }
